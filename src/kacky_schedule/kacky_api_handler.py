...
TEST_API_RESPONSE = {
    "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $g7 - Server 1": {
        "jukebox": [],
        "recently_played": [
            "sbQZsoFMQ0yi7I4zrZn6wn06_d1",
            "eDjkDjgh2dOMzmSilAH6nWzWBv",
            "Vb1uh9lRARwGVSdbx1g6LC4UKdi",
            "o2Vneej0ouvNTav5LJTuygXaXz1",
            "kprNTNffW1tnZsojh69vFAqxeqg",
        ],
        "current_map": "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $0f4#223",
        "time_played": "854",
    },
    "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $g7 - Server 2": {
        "jukebox": [],
        "recently_played": [
            "oPTugNFan8Q22EFeRt710HnclZm",
            "yJ3V2YlglO7vusSGI5nVeCP8aFf",
            "7tjAh6BpWj6eSPZehsutTwELAfk",
            "nEOGfQ8W1iYS6PiFVqB8Eda_4bc",
            "7ykxjr_DvbWBtZKDlsm9wTnzji2",
        ],
        "current_map": "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $0f4#251",
        "time_played": "702",
    },
    "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $g7 - Server 3": {
        "jukebox": [],
        "recently_played": [
            "gY_wuO4bY12OuT1WqIOWTZ2P4Da",
            "4e8NjxQQrC3Hy_7cBBOmGhbYuyi",
            "kwuHbknIiJGl6JT0bIQOixCXzYc",
            "ESXoxCHq2nzWaTPSrFm5xbrNzld",
            "f86RbkuB7YLcTj0uFAPfNuECsQi",
        ],
        "current_map": "$o$i$a00K$a60a$aa0ck$0a0iest Kack$a00$a00y $0f4#234",
        "time_played": "412",
    },
}
# simo_900
TEST_LOGIN_RESPONSE_SIMO = {
    "finishes": 75,
    "mapids": [
        "201",
        "202",
...
TEST_LEADERBOARD_RESPONSE = [
    [1, "sexyboii", "plastorex", 3.7],
    [2, "$abcsimo_$777900", "simo_900", 4.2],
    [7, "k$cba*$z$gm", "amgreborn", 69.69],
    [17, "corkscrew", "corkscrew", 134.0],
]
...
class KackyAPIHandler:
    ...
    def update_server_info(self):
        if self.config["testing_mode"]:
            self.logger.warning("TESTING MODE - USEING TEST DATA")
            krdata = TEST_API_RESPONSE
        elif not self.last_server_update < datetime.datetime.now() - datetime.timedelta(
            minutes=1
        ):
    ...
    def get_fin_info(self, tmlogin):
        try:
            findata = requests.post(
                "https://kk.kackiestkacky.com/api/",
                data={"login": tmlogin, "password": self.api_pwd},
            ).json()
        except ConnectionError:
            self.logger.error("Could not connect to KK API!")
            flask.render_template(
