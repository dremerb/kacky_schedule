...
class TMstr:
    def __init__(self, string):
        ...
        for f in found:
            ...
            htmlstr = re.sub(f"\${f[1:]}", f"</span><span style='color:#{f[1:]}'>", htmlstr)
        # remove leading "</span>" and add a "</span>" at the end
        htmlstr = re.sub("</span>", "", htmlstr, count=1)

        # replace style formatters
        formatters = {"w": '<span style="letter-spacing: +0.1em;font-size:larger">',        # wide font
                      "n": '<span style="letter-spacing: -0.1em;font-size:smaller">',       # narrow font
        ...
        formatmatches = re.findall("|".join([f"\${str.lower(f)}|\${str.upper(f)}" for f in formatters.keys()]), htmlstr)
